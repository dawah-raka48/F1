...
function build(a){
 ...
 const body=a.rows.map((r,i)=>'<tr><td class="c-no">'+(i+1)+'</td><td class="c-name">'+esc(r.name)+'</td>'+r.marks.map(x=>'<td class="c-mark">'+(x===true?'✓':x===false?'✗':'')+'</td>').join('')+'<td class="c-average"><b>'+esc(r.average||'—')+'</b></td><td class="c-notes">'+esc(r.notes||'')+'</td></tr>').join('');
 const averages=subjects.map((_,j)=>{const n=a.rows.reduce((s,r)=>s+(r.marks[j]===true?1:0),0);return Math.round(n/a.rows.length*100)+'%'}).join('</td><td>');
 return '<div class="pdf-sheet" id="sheet">'+
 '<div class="pdf-head"><div class="pdf-school">Halley International School – HIS</div><div class="pdf-title">Weekly Student Assessment – '+esc(a.className)+'</div></div>'+
 '<div class="pdf-info"><div><span>Week</span><b>'+esc(a.week)+'</b></div><div><span>From</span><b>'+esc(a.from||'—')+'</b><span>To</span><b>'+esc(a.to||'—')+'</b></div><div><span>Teacher Name</span><b>'+esc(a.teacher)+'</b></div></div>'+
 '<div class="pdf-scale"><b>Rating Scale:</b> 5 = Excellent &nbsp; 4 = Very Good &nbsp; 3 = Good &nbsp; 2 = Fair &nbsp; 1 = Needs Improvement</div>'+
 ...
 '<tbody>'+body+'</tbody><tfoot><tr><td colspan="2"><b>Class Average</b></td><td>'+averages+'</td><td></td><td></td></tr></tfoot></table>'+
 '<div class="pdf-footer">Halley International School – HIS &nbsp; | &nbsp; Weekly Student Assessment</div></div>';
}
