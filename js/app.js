...
function setButtonBusy(button,text='Saving...'){if(!button)return;button.dataset.originalHtml=button.innerHTML;button.disabled=true;button.classList.add('is-busy');button.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> '+text}
function restoreButton(button){if(!button)return;button.disabled=false;button.classList.remove('is-busy');if(button.dataset.originalHtml)button.innerHTML=button.dataset.originalHtml}
...
async function saveClass(e,editId){e.preventDefault();const submit=e.submitter||document.querySelector('#classForm button[type="submit"]');if(submit?.disabled)return;setButtonBusy(submit,'Saving...');const d=getData(),id=document.getElementById('mClassId').value.trim(),name=document.getElementById('mClassName').value.trim();if(!id||!name){restoreButton(submit);return}if(d.classes.some(c=>c.id.toLowerCase()===id.toLowerCase()&&c.id!==editId)){toast('Class code already exists','error');restoreButton(submit);return}if(editId){const c=d.classes.find(x=>x.id===editId);c.id=id;c.name=name}else d.classes.push({id,name,students:[]});try{await saveData(d);closeModal();toast('Saved successfully')}catch(error){restoreButton(submit)}}
...
async function deleteClass(id){const d=getData(),c=d.classes.find(x=>x.id===id);if(!c)return;const ok=await platformConfirm('Delete '+c.name+' and its student list?','Delete Class','This action cannot be undone.');if(!ok)return;d.classes=d.classes.filter(x=>x.id!==id);if(!d.classes.length)d.classes=[{id:'1A',name:'Class 1A',students:[...defaultStudents]}];await saveData(d);closeModal();toast('Class deleted')}
...
async function addStudent(classId){const button=document.getElementById('addStudentBtn');if(button?.disabled)return;setButtonBusy(button,'Saving...');const input=document.getElementById('studentInput'),name=input?.value.trim(),d=getData(),c=d.classes.find(x=>x.id===classId);if(!c||!name){restoreButton(button);return}if(c.students.some(s=>s.toLowerCase()===name.toLowerCase())){toast('Student already exists','error');restoreButton(button);return}c.students.push(name);try{await saveData(d);refreshStudentModal(classId);toast('Saved successfully')}catch(error){restoreButton(button)}}
...
async function deleteStudent(classId,i){const d=getData(),c=d.classes.find(x=>x.id===classId);if(!c)return;const student=c.students[i];const ok=await platformConfirm('Delete '+student+' from '+c.name+'?','Delete Student','The student will be removed from the current class list.');if(!ok)return;c.students.splice(i,1);await saveData(d);refreshStudentModal(classId);toast('Student removed')}

function platformConfirm(message,title='Confirm Action',hint='Please confirm this action.'){
 return new Promise(resolve=>{
  const root=document.getElementById('modalRoot');
  if(!root){resolve(window.confirm(message));return}
  const safeMessage=esc(message),safeTitle=esc(title),safeHint=esc(hint);
  root.innerHTML='<div class="modal-backdrop platform-confirm-backdrop"><div class="confirm-card" role="dialog" aria-modal="true" aria-labelledby="confirmTitle"><div class="confirm-icon"><i class="fa-solid fa-triangle-exclamation"></i></div><div class="confirm-content"><span class="section-kicker">CONFIRMATION</span><h2 id="confirmTitle">'+safeTitle+'</h2><p class="confirm-message">'+safeMessage+'</p><p class="confirm-hint">'+safeHint+'</p></div><div class="confirm-actions"><button type="button" class="btn soft" id="confirmCancel"><i class="fa-solid fa-xmark"></i> Cancel</button><button type="button" class="btn danger confirm-delete" id="confirmOk"><i class="fa-solid fa-trash"></i> Delete</button></div></div></div>';
  const finish=value=>{root.innerHTML='';resolve(value)};
  root.querySelector('#confirmCancel').onclick=()=>finish(false);
  root.querySelector('#confirmOk').onclick=()=>finish(true);
  root.querySelector('.platform-confirm-backdrop').onclick=e=>{if(e.target===e.currentTarget)finish(false)};
  document.addEventListener('keydown',function onKey(e){if(e.key==='Escape'){document.removeEventListener('keydown',onKey);finish(false)}},{once:true});
  setTimeout(()=>root.querySelector('#confirmCancel')?.focus(),30);
 });
}
