...
function getData(){
  try{
    ...
    if(!raw||!Array.isArray(raw.classes)||!Array.isArray(raw.assessments)) throw new Error('invalid');
    if(raw.classes.length===0){
      raw.classes=[{id:'1A',name:'Class 1A',students:[...defaultStudents]}];
    }else{
      raw.classes=raw.classes.map(c=>({id:String(c.id||''),name:String(c.name||c.id||'Class'),students:Array.isArray(c.students)?c.students.filter(Boolean):[]}));
    }
    return raw;
  }catch(e){
...
function saveData(d){localStorage.setItem(KEY,JSON.stringify(d));renderDashboard()}
...
function saveClass(e,editId){e.preventDefault();const d=getData(),id=document.getElementById('mClassId').value.trim(),name=document.getElementById('mClassName').value.trim();if(!id||!name)return;if(d.classes.some(c=>c.id.toLowerCase()===id.toLowerCase()&&c.id!==editId)){toast('Class code already exists','error');return}if(editId){const c=d.classes.find(x=>x.id===editId);c.id=id;c.name=name}else d.classes.push({id,name,students:[]});saveData(d);closeModal();toast(editId?'Class updated successfully':'Class created successfully')}
...
function deleteClass(id){const d=getData(),c=d.classes.find(x=>x.id===id);if(!c)return;if(!confirm('Delete '+c.name+' and its student list?'))return;d.classes=d.classes.filter(x=>x.id!==id);if(!d.classes.length)d.classes=[{id:'1A',name:'Class 1A',students:[...defaultStudents]}];saveData(d);closeModal();toast('Class deleted')}
...
function addStudent(classId){const input=document.getElementById('studentInput'),name=input?.value.trim(),d=getData(),c=d.classes.find(x=>x.id===classId);if(!c||!name)return;if(c.students.some(s=>s.toLowerCase()===name.toLowerCase())){toast('Student already exists','error');return}c.students.push(name);saveData(d);refreshStudentModal(classId);toast('Student added successfully')}
function renameStudent(classId,i,name){const d=getData(),c=d.classes.find(x=>x.id===classId);if(!c)return;name=name.trim();if(!name)return;if(c.students.some((s,n)=>n!==i&&s.toLowerCase()===name.toLowerCase())){toast('Student already exists','error');return}c.students[i]=name;saveData(d);toast('Student name updated')}
function deleteStudent(classId,i){const d=getData(),c=d.classes.find(x=>x.id===classId);if(!c)return;if(!confirm('Delete '+c.students[i]+' from '+c.name+'?'))return;c.students.splice(i,1);saveData(d);refreshStudentModal(classId);toast('Student removed')}
